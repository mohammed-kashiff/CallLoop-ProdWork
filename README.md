...
| `ANTHROPIC_API_KEY` | Claude scoring |
| `CORS_ORIGINS` | `https://callloop-web.onrender.com,https://call-loop.com,https://www.call-loop.com,https://commandcenter.call-loop.com` (comma-separated; never `*`) |
...
**Decision: shared-build, not a separate admin compile.** `commandcenter.call-loop.com` is a second custom domain on the same `callloop-web` static site. The JS bundle is the customer app; the hostname only changes routing and chrome (land on `/admin`, hide Loop nav, no public signup). Auth is still the same Supabase Bearer JWT and `require_platform_admin` — no extra or weaker gate. A compromised customer-app bundle can still reach admin routes in that file; this is a dedicated URL, not a hardened origin boundary. A separate Vite app would be the isolation upgrade if we need one later.

**Renamed from `idb.call-loop.com`** — this is a full cutover, not a second domain. To finish (dashboard, not this repo):

1. Add custom domain `commandcenter.call-loop.com` on the `callloop-web` static site.
2. Point the DNS CNAME at that site.
3. Confirm `CORS_ORIGINS` on `callloop-prodwork` includes `https://commandcenter.call-loop.com` (blueprint `render.yaml` already lists it; dashboard env wins if it overrides).
4. Add `https://commandcenter.call-loop.com/**` to the Supabase Auth redirect allow list.
5. Once traffic has moved, remove the `idb.call-loop.com` custom domain, its DNS record, its Supabase Auth redirect entry, and drop it from `CORS_ORIGINS` if the dashboard env still lists it.
