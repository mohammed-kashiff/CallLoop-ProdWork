...
| GET | `/api/team-performance` | Org-scoped Team Performance rollup (`?days=` 1–365, default 30). Owner/Manager see every teammate (`view_scope=team`); a member sees only their own row (`own`). Ticket and call Top Strength/Gap (mode of `ticket_audit_summary` pickers). Coverage = scored vs total in the window. `heatmap.tickets` / `heatmap.calls` = per-dimension pass rates. |
