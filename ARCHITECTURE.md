...
| `ticket_audit_summary.py` | IN-12. `top_strength()`/`top_gap()` (deterministic, no Claude call — highest-weighted pass/fail dimension respectively), `generate_audit_summary()`, and `enrich()` (attaches those three onto a scorecard dict). Built fresh from CallLoop's own per-dimension scoring output only — never Intercom's `conversation_summary`, never PyAI's Recap. GET ticket / GET mine / POST score / Team Performance call rollups all go through this. |
