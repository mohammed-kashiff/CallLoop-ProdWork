...
| `ticket_rescore_jobs.py` | **New, IN-27 follow-on (2026-09-17).** Internal, platform-admin-only bulk re-score by org + date range (`start_backfill()`/`get_job()`, wired at `ticket_score_api.py`'s two `/api/admin/.../ticket-rescore-jobs` routes) — built to actually fix already-scored tickets after a scoring bug is corrected (IN-27's own `is_image_description` fix needed exactly this), without touching `enable_ticket_rescoring` in either direction; that flag keeps gating the everyday customer-facing rescore feature exactly as before, unrelated to this tool. Runs as a `threading.Thread` in the same process (no queue infra — single Render instance, free plan) with progress/ETA in an in-memory dict — lost on restart, and only one job may run at a time since ticket scoring is itself fully serial (one real Claude call per rubric dimension per agent). The item count and initial ETA are computed from real `resolved_agent_ids()` data before any Claude call, not a guess; the ETA then recomputes from real elapsed time as the job runs. `completed_items` counts every attempted item, success or failure (the progress/ETA denominator), with per-ticket failures collected separately in `errors` rather than aborting the whole job. |
...
| POST | `/api/admin/orgs/{org_id}/ticket-rescore-jobs` | **Platform-admin only.** IN-27 follow-on (`ticket_rescore_jobs.py`). Body `{from_date, to_date}` — starts a background job that re-scores every resolved agent on every ready ticket in that org/date-range, overwriting whatever's stored. Independent of, never touching, the org's own `enable_ticket_rescoring` flag — an internal correction tool, not a way around a customer's rescoring setting. Returns the job's initial snapshot (real item count, real ETA from `resolved_agent_ids()`, no Claude call needed to plan it) immediately; refuses a second job while one is running (ticket scoring is fully serial — two at once only makes both slower). |
| GET | `/api/admin/ticket-rescore-jobs/{job_id}` | **Platform-admin only.** Poll a backfill job's progress — `completed_items`/`total_items`/`estimated_seconds_remaining` (recomputed from real elapsed time as it runs, not a static guess) plus any per-ticket errors. In-memory only (no queue infra in this codebase) — a job's state is lost on process restart. |
...
| GET | `/api/team-performance` | Org-scoped Team Performance rollup (`?days=` 1–365, default 30). Owner/Manager see every teammate (`view_scope=team`); a member sees only their own row (`own`). Ticket and call Top Strength/Gap (mode of `ticket_audit_summary` pickers). Coverage = scored vs total in the window. |
