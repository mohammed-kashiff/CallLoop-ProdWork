...
| `team_performance.py` | Team Performance Dashboard rollups. Simple org-scoped SQL (no Redis, no job): ticket avgs from `ticket_audits` (one row per agent), call avgs from the latest `audits` row per call joined to `calls.agent_user_id` (IN-22). Ticket Top Strength/Gap is the mode of `ticket_audit_summary.top_strength`/`top_gap` across that agent's stored findings in the window. **No call-side Top Strength/Gap** — that module does not exist. Display names come from `org_members` first/last, never email. Unassigned calls (`agent_user_id` NULL) are a manager-only bucket. |
...
| GET | `/api/team-performance` | Org-scoped Team Performance rollup (`?days=` 1–365, default 30). Owner/Manager see every teammate (`view_scope=team`); a member sees only their own row (`own`). Ticket Top Strength/Gap only — no call-side equivalent. |
...
- **`TicketAudit.tsx` doesn't render IN-12's `audit_summary`/`top_strength`/`top_gap` yet** — `POST /api/tickets/{ticket_id}/score` returns all three per agent today (`agents[].audit_summary`/`top_strength`/`top_gap`, TA-21's rebuild), but the frontend's types don't declare them and nothing displays them. Backend-complete, UI not started. The Team Performance page *does* surface per-agent ticket Top Strength/Gap (mode across the window). Call-side Top Strength/Gap is still unbuilt — there is no call equivalent of `ticket_audit_summary.py`.
