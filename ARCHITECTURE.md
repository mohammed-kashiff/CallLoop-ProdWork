...
| GET | `/api/integrations/intercom/connect` | Starts the OAuth flow — 302 to Intercom's authorize URL, `state` HMAC-signed with the caller's org_id. 403 when `enable_intercom_integration` is off for the org (checked after the 503 app-not-configured case). DELETE `/api/integrations/intercom` (disconnect) is never gated. |
