...
| `0027_platform_admins_revoke_default_grant` | `platform_admins` inherited SELECT/INSERT/UPDATE/DELETE from 0005's `ALTER DEFAULT PRIVILEGES` (applies to every new public-schema table unless revoked — caught live, right after 0026 deployed, by a test asserting the grant was empty). RLS alone already blocked `callproof_app` (verified: SELECT returned zero rows, INSERT raised `InsufficientPrivilege`), but this explicitly `REVOKE`s the grant so it doesn't rely on RLS alone. |
