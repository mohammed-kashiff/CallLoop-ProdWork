...
>
> **Pipeline trails (Command Center, platform-admin only):** a per-call
> trail (`backend/call_trail.py`, `call_pipeline_events`, migration
> `0021`, AC-24) and a per-ticket trail (`backend/ticket_trail.py`,
> `ticket_pipeline_events`, migration `0045`). Every stage of ingest ->
> score -> serve, including per-criterion dispatch and every failure with
> its cause, as queryable rows rather than just log lines. Call logs /
> Ticket logs -> a row's "View" trail link
> (`frontend/src/pages/CallTrail.tsx`, `TicketTrail.tsx`;
> `GET /api/admin/calls/{call_id}/trail`,
> `GET /api/admin/tickets/{ticket_id}/trail`). "Result displayed" is
> tracked as "the audit/score API actually served it" (`result_served`) —
> the practical, backend-observable proxy; there's no way to know when
> something rendered on a screen. Ticket scoring is per-agent, so
> criterion rows put `agent_user_id` in `detail` (never transcript quotes).
> Each ticket-trail step also lists the HTTP APIs that step called as
> `{method, endpoint}` templates in `detail["apis"]` (no live ids, query
> strings, or attachment URLs).
> Both writers are best-effort and never `bypass_rls`; `org_id` on the
> GET is caller-supplied from the logs search.
>
...
> boundary already enforced for `findings`/`spans`. Frontend now consumes
> these on Ticket Audit, My contributions, Team Performance (mode across
> the window, including call findings via the same pickers), and Training
> drills.
>
...
| `auth.py` | Verifies Supabase JWTs, `ensure_membership()`, `ensure_placeholder_org()` — idempotent seed of `DEFAULT_ORG_ID` for webhook/CLI/usage fallbacks only (not signup) — `require_platform_admin()`, the internal admin-console gate, and `require_owner()`, still owner-only for everything not explicitly listed below. **AC-56 (Roles: Owner, Manager, Agent), 2026-09-16**: `org_members.role` now has three real values (migration `0038`, widening a hard `CHECK` in place since `0004_org_members.py`) — `is_org_manager()`/`is_owner_or_manager()`/`require_owner_or_manager()` are the narrow grant: Owner and Manager get identical access to the ticket-audit team view (TA-12, `ticket_api.py`/`ticket_score_api.py`), the self-serve rubric builder (call and ticket, `rubric_builder.py`/`ticket_rubric_builder.py`), and Team Performance team scope (`GET /api/team-performance`) — everything else (`org_roles.py`'s promote/demote, alias mapping, admin routes) stays `is_org_owner()`/`require_owner()`, which never returns true for a Manager. `is_platform_admin()` (0026) ORs the static `PLATFORM_ADMIN_EMAILS` env var with a DB-resolved flag: `ensure_membership()` calls `is_platform_admin_email()` (SECURITY DEFINER) once per request and stashes the result on `request.state.is_platform_admin_db` — no extra DB round trip, since `ensure_membership` already runs once per authenticated request regardless. `_auth_failure()` (2026-09-06) logs `event=auth_failed` under `callproof.auth` — previously logged nothing at all on any 401/503, so failed-auth traffic was invisible in Better Stack. A 401 (expired/missing/invalid token) logs at WARNING since it's routine traffic and shouldn't inflate error-rate alerts; a 503 (auth not configured) logs at ERROR since that's a real misconfiguration. |
| `org_roles.py` | **New, 2026-09-16 (AC-56/AC-58).** The account owner promotes/demotes their own team members to/from Manager — `list_team()` (for the Profile page's "Your team" section) and `set_member_role()`, both owner-only (`auth.require_owner`, not `require_owner_or_manager` — a Manager can never create another Manager). `org_members` has no RLS at all (`0005_rls.py`'s own docstring: "org_members is not RLS'd... blanket GRANT to callproof_app instead"), so every query here scopes `org_id` from the caller's own verified `auth.org_id_from_request()`, never client input or the path's `target_user_id` alone — proven live by a test that plants a member in a second org and confirms `set_member_role()` 404s rather than reaching across tenants. `set_member_role()` refuses to touch a row whose current role is `"owner"` and refuses to ever assign anything but `"manager"`/`"member"` — this endpoint can never mint a second owner or transfer ownership (both explicitly out of scope for AC-56). |
...
| `org_features.py` | AC-4/AC-5: `features_for_org()` (read, org-scoped, defaults missing keys to enabled) and `set_feature()` (upsert, admin-gated caller). `enable_call_rescoring` (off by default) gates whether an already-audited call can ever be re-scored — see `api._load_or_compute_audit`. **TA-22/TA-32/TA-33 (Launch Gating, 2026-09-16)**: `show_ticket_audit_nav` flipped to **on by default** for new orgs — it was gating on the Real Ticket QA Rubric, Roles, and Per-Agent Ticket Audit Rebuild epics, all now shipped. `enable_ticket_rescoring` stays off by default, but that's now a documented permanent decision (same reasoning as `enable_call_rescoring`: score stability, real Claude cost per re-run), not a leftover launch-gating caution flag. AC-36: `FEATURE_DEFINITIONS` is now the single source of truth per flag (label, description, `risk: "low"\|"medium"\|"danger"`, default_enabled) — `FEATURE_KEYS`/`DEFAULT_OFF_KEYS` are *derived* from it, not maintained separately, so they can't drift apart again. `feature_definitions()` serves this metadata for Command Center's Flags tab to group Low/Medium/Danger and gate a confirmation prompt on danger-tier toggles generically — add a flag here with a risk tier and no frontend change is needed for it to show up correctly grouped. Today `enable_bulk_call_clear` is the only `danger`-tier flag (hard-delete-adjacent, org-wide); `use_selfhosted_transcription`/`enable_call_rescoring`/`enable_ticket_rescoring` are `medium` (real behavior change, not destructive); the rest are `low` (cosmetic UI/nav toggles). **2026-09-18**: `show_growth_tools_nav` (one bundled sidebar block for Feedbacks/Churn Risk/Integrations/Training, named after none of them precisely) was split into `show_churn_feedback_nav`, `show_integrations_nav`, and `show_training_nav` — each flag now names exactly what it controls (`Sidebar.tsx`). The three orgs with an explicit (all-`true`) row under the old key were migrated directly in the DB; no Alembic migration, since `feature_key` is a free-text column. Also added: `enable_justcall_integration`/`enable_intercom_integration` (medium risk, on by default) — gate the connect routes, the manual sync action, and each provider's background poller independently per org (`api.py`'s `_justcall_poll_once`/`_intercom_poll_once`, both skip a disabled org rather than syncing it; disconnect is never gated, so a disabled org can still remove its own stored credentials). New orgs created via signup (`auth.ensure_membership` → `org_features.seed_new_org_defaults()`) get an explicit `enable_justcall_integration=false` row at creation — JustCall starts opted-out for new signups; every existing org is unaffected (no row written for them) and Intercom is not special-cased. |
...
| `product_events.py` | AC-43/44/45 (observability PRD §4): `track_event()` persists a fifth instance of the append-only, org-scoped events-table pattern (`impersonation_log`/`password_reset_events`/`org_features_history`/`call_trail`). `ALLOWED_EVENTS` is a deliberately closed allowlist (the PRD's own "dumping ground" risk) split into `BACKEND_WIRED_EVENTS` (called directly from the route/service already handling that action — `ticket_api.py`, `rubric_builder.py`, `ticket_rubric_builder.py` (`ticket_rubric_saved`, added TA-24 follow-on), `api.py`'s upload/flag/solve/feedback/stakeholder-email routes) and `FRONTEND_ONLY_EVENTS` (page views, `session_started` — reachable only via `POST /api/events`, which rejects any backend-wired name so the browser can never fabricate one). Best-effort by design: never raises, logs and returns on any failure — a telemetry bug must not break the feature it's attached to. |
...
| `audit_store.py` | Reads/writes `audits` and `rubrics` rows; seeds the legacy rubric for new orgs. `insert_weighted_version()` (CR-13, Command Center reweighting) and `insert_custom_definition()` (self-serve builder, arbitrary dimension set) are separate functions on purpose — same versioning discipline, kept apart so the admin-only tool stays untouched by the self-serve one. `fetch_active_rubric()` explicitly excludes `definition->>'kind' = 'ticket'` rows (TA-13) — the ticket engine's "Ticket QA" rubric lives in this same table with no schema change, sharing the org-wide `is_active` column, so this exclusion is what keeps a real call from ever getting handed the ticket rubric. **Found live (2026-09-16): the exclusion existed only on this read path.** `save_named_rubric()`'s "deactivate whatever else is active for this org" step, `list_rubric_lineages()`, and `fetch_rubric_by_name()` had no exclusion at all — saving/activating a call rubric through the self-serve builder silently deactivated an org's active Ticket QA rubric too (a real ticket-scoring org would then fall back to a fresh default, quietly losing any ticket customization), and a call rubric library listing could show the ticket rubric as if it were one of the team's own saved call rubrics. All three now carry the same `COALESCE(definition->>'kind', 'call') <> 'ticket'` exclusion as `fetch_active_rubric()`; `rubric_builder.py`'s `current_rubric()`/`_resolve_default_name()` (its own raw queries, not routed through this module) got the matching fix. **Generalized further, 2026-09-16 (TA-24 follow-on, `ticket_rubric_builder.py`):** these same four functions (`save_named_rubric`, `list_rubric_lineages`, `fetch_rubric_by_name`, `activate_rubric_by_name`) now take a `kind: str = "call"` parameter — `= kind` instead of the literal `<> 'ticket'` — so the exact same code serves both engines instead of a second copy; the default reproduces the original call-only behavior exactly, and `rubric_builder.py`'s call sites now pass `kind="call"` explicitly. **Found live in the same pass: `activate_rubric_by_name()` had no kind exclusion at all** — not even the old `<> 'ticket'` pattern the other three already carried — a second, sibling instance of the TA-35 cross-engine wipeout that fix session missed; also fixed the "find this name's latest version" lookup and the version-numbering `MAX(version)` query in `save_named_rubric()`, both previously unscoped by kind (a same-named rubric across kinds could have resolved to the wrong engine's row or shared a version counter). |
| `rubric_builder.py` | Self-serve rubric builder (customer-facing, gated by `auth.require_owner_or_manager` — AC-56/AC-61, 2026-09-16, was owner-only — not admin): a team's own mix of built-in dimensions (reused unchanged from `rubric.json` unless the team edits their criteria text, which converts that one to a custom dimension — a built-in's deterministic logic isn't rewritable by text) and free-text custom ones (`method: "custom_llm"`, Claude-judged). A **multi-rubric library**: teams save several independently-versioned named rubrics (`audit_store.save_named_rubric`/`list_rubric_lineages`/`activate_rubric_by_name`, all called with `kind="call"` explicitly since TA-24's follow-on generalized them), at most one active org-wide at a time within that kind — same `rubrics` schema, no migration. Deliberately separate from `admin_console.py`. Its ticket-side twin is `ticket_rubric_builder.py`. |
...
| `transcribe_selfhosted.py` | CL-38. Self-hosted ASR + diarization (torch/pyannote), sibling to `transcribe.py`'s PyAI Hear path. Not wired into live routes (CL-39) — returns the exact same `{segments, words, speakers, text, audio_seconds}` shape Hear produces, so `save_transcript`/`replace_transcript`/the segments table/the frontend never see which engine ran. Models load once per process, lazy + locked; requires `HF_TOKEN` on the host. |
| `transcribe_selfhosted_remote.py` | CL-40 (Modal GPU). HTTP client calling the self-hosted engine remotely instead of running torch/pyannote in-process — that once took down the whole API by starving/exhausting the box's resources. Same `(job_id, result, mode)` return shape as `transcribe_selfhosted()`, so `transcribe_audio()`'s dispatch doesn't care which ran. No auth token — Modal's endpoint is a plain public HTTPS URL. `REQUEST_TIMEOUT=1800` spans Modal's own 150s-per-hop limit plus its 303-redirect-to-polling-URL pattern (`httpx`'s `follow_redirects` handles the hops automatically). |
| `selfhosted_service.py` | CL-40. Thin FastAPI wrapper exposing `transcribe_selfhosted()` over HTTP, meant to run on Cloud Run — never on the box serving live API traffic, which is the whole point of splitting it out. No transcription logic of its own. |
...
| `ticket_rubric.py` | TA-7/TA-13/**TA-24 (2026-09-16)**. `TICKET_QA_DIMENSIONS` — the real, final five LLM-judged dimensions (Sellable Release PRD §3), replacing TA-7's six-dimension scaffold outright: Problem Diagnosis 22%, Resolution Correctness 27%, Communication Clarity 17%, Tone & Empathy 17%, Ownership & Handoff Quality 17% (rescaled proportionally from TA-23's PRD-specified weights, which summed to 90%, not 100%). `ensure_ticket_rubric()`/`fetch_active_ticket_rubric()` store/read the org's "Ticket QA" rubric as a real `rubrics`-table row (PRD §10, no schema change — a `definition->>'kind' = 'ticket'` marker distinguishes it from call rubrics). `evaluate_response_timeliness()` is the sixth, deterministic dimension — real elapsed time between a customer message and the next agent reply, from `sent_at`; not folded into `score_ticket_for_agent()`'s weighted score, returned as its own finding instead. **Rebuilt per-agent, TA-27 (2026-09-16)**: gained `target_agent_user_id: str | None = None` — when set, only a gap ending in that specific agent's own reply counts toward `worst_gap`, but `waiting_since` still resets whenever *any* agent replies (a customer's wait genuinely ends regardless of who answered; only whether the gap counts toward *this* agent's score differs). `None` preserves the old ticket-wide "worst gap by anyone" behavior. IN-9's `customer_facing_only` reasoning carried forward onto the new set: only Communication Clarity and Tone & Empathy have it; the other three deliberately can see internal Intercom notes (Resolution Correctness explicitly needs to — the epic's own motivating example was a decisive decision delivered entirely through a note). No dimension carries a `scaffold` flag any more; the API's `rubric_scaffold: True` response field was removed (`ticket_score_api.py`) and the stale "not the final rubric" banner was removed from `TicketAudit.tsx`. |
| `ticket_rubric_builder.py` | **New, 2026-09-16 (TA-24 follow-on).** Self-serve TICKET rubric builder — the exact `rubric_builder.py` mechanism (mix of built-in + free-text custom dimensions, weights summing to 100, named/saved, multiple lineages, choose-which-is-active), reused for `kind="ticket"` rows via `audit_store.py`'s now-generalized `save_named_rubric`/`list_rubric_lineages`/`fetch_rubric_by_name`/`activate_rubric_by_name` (each grew a `kind: str = "call"` parameter — the default reproduces the original call-only behavior exactly; this module passes `kind="ticket"`). Builtins come from `ticket_rubric.TICKET_QA_DIMENSIONS`, not `qa_v8`; the wire shape is the flat dimensions list `ticket_scoring.score_ticket()` reads, not the call engine's two-bucket shape; `"response_timeliness"` is a reserved id — always computed deterministically and appended outside the weighted score, never a pickable weighted dimension. Owner-or-manager-gated (`auth.require_owner_or_manager` — AC-56/AC-61, 2026-09-16, was owner-only) exactly like the call builder. Wired at `/api/tickets/rubric/builder` (GET/POST) and `/api/tickets/rubrics[/{name}[/activate]]` (`ticket_score_api.py`), deliberately distinct from the pre-existing read-only `/api/tickets/rubric` viewer route. **Found live while building this (2026-09-16): `audit_store.activate_rubric_by_name()` had no kind exclusion at all — not even the `<> 'ticket'` pattern the other three functions already carried — the exact TA-35 cross-engine wipeout, in a sibling function that fix session missed. Activating a call rubric could have silently deactivated an org's active Ticket QA rubric (and vice versa), and a same-named rubric across kinds could have activated the wrong engine's row outright.** Fixed by scoping both its "find this name's latest version" lookup and its "deactivate whatever else is active" step by kind, same as the other three functions — proven by a live test that saves a call rubric, saves *and activates* a ticket rubric through this new module, and confirms the call rubric survives both. |
...
| `ticket_api.py` | TA-9, rebuilt TA-21/TA-29/TA-30 (2026-09-16). `/api/tickets` HTTP surface. `POST /api/tickets/upload` accepts a PDF and hands it to `ingest_ticket_pdf` — not `/api/upload`. JWT org_id only. `GET /api/tickets/{id}` and `GET /api/tickets/mine` now always return the **full thread**, regardless of role — TA-30 fixed a real bug found live where a non-manager's thread got stripped to only their own span, so an agent reviewing their own ticket couldn't see what the customer originally asked. Only which agent's scorecard(s) a viewer sees is still access-controlled (`ticket_permissions.filter_audits_for_viewer`); `own_span_seqs` (also `ticket_permissions.py`) tells the frontend which turns to *highlight* as the viewer's own in that full thread. Manager check is `auth.is_owner_or_manager()` (AC-60, 2026-09-16 — was owner-only). |
| `ticket_permissions.py` | TA-12, rebuilt TA-21/TA-29/TA-30 (2026-09-16). `filter_turns_for_viewer`/`filter_spans_for_viewer`/`filter_findings_for_viewer` were **deleted outright** — see `ticket_api.py`'s row above for why. What remains: `own_span_seqs(turns, viewer_user_id)` (which turns in the always-full thread are the viewer's own, for highlighting) and `filter_audits_for_viewer(audits, *, viewer_user_id, is_manager)` — a manager (`auth.is_owner_or_manager()`) sees every agent's independent scorecard (TA-28's one-row-per-agent model); anyone else sees only their own, never a teammate's individual score even on a thread they share. |
| `ticket_scoring.py` | TA-6, rebuilt TA-21/TA-25/TA-26/TA-28 (2026-09-16). **The core architectural fix for a bug found live in the pilot: two agents on the same ticket got the identical score**, because v1 scored the whole thread once per rubric dimension and post-hoc attributed each finding's evidence to whichever agent's span it landed in — a criterion whose only citable evidence sat in Agent B's turns gave Agent A no finding at all, not a pass or fail. Replaced with one independent Claude call per dimension per agent: `format_turns_for_agent()`/`_question_for_agent()` label the transcript so the model judges only the agent under review (a teammate's turns are shown for context, explicitly marked "do not judge them"); `_seq_in_agent_span()` checks cited evidence against real span boundaries and `evaluate_criterion_for_agent()` downgrades a verdict to `"error"`/`evidence_verified=False` when evidence is verified but falls outside the target agent's own span, rather than silently crediting it to the wrong person. `resolved_agent_ids(turns)` lists the distinct real agents on a ticket — the only ones who get their own scorecard; a turn with no resolved identity is still part of the thread everyone sees but has no real person to score. `score_ticket_per_agent(turns, dimensions, *, only_agent_ids=None, ...)` is the entrypoint, returning one `{agent_user_id, score, findings, spans}` dict per target agent. `primary_owner()` and the old whole-ticket `score_ticket()` were **removed entirely** — there is no single "owner" of a ticket any more, only agents each independently scored. `agent_spans()` itself is unchanged (span-splitting logic didn't need to change, only what's done with it). IN-9: `_scoreable_turns()` still filters out `internal_contribution` turns before both prompting and evidence validation for a `customer_facing_only` dimension — filters, never renumbers, so `evidence_seq` still indexes the real thread. **Per-criterion marks (2026-09-16)**: `run_ticket_wave_for_agent()` also attaches `earned` per finding — `weight` × the verdict's points fraction (pass=full, partial=half, fail=0), via the new `_earned_weight()` helper (also reused by `_numeric_score()`, one source of truth for the points rule). `None` for a finding not part of the weighted score (not_applicable/error, or a weightless dimension like Response Timeliness). Read straight off whatever the org's rubric weight is at scoring time, so a rubric edit is picked up on the next score, same as the score itself — a *stored* finding keeps whatever weight it was scored against until re-scored. `TicketAudit.tsx`/`MyTicketContributions.tsx` render it as `{earned}/{weight}` next to the verdict badge. |
| `ticket_score_api.py` | TA-10/TA-11/TA-13/IN-12/TA-24, rebuilt TA-21/TA-25/TA-28 (2026-09-16). `POST /api/tickets/{ticket_id}/score` resolves every real agent on the ticket (`resolved_agent_ids`) and scores each independently against the org's real "Ticket QA" rubric — 400 if none are resolved yet ("map agent names first"). **The rescoring guard is now per-agent, not per-ticket (TA-28)**: a stored row for Agent A blocks a re-run on A without `?refresh=true` + `enable_ticket_rescoring` (403 otherwise, same as before), but a newly-resolved Agent B with no row yet still gets a real first score on a plain POST even if Agent A was already audited — that's a genuine first score for B, not a re-score of already-audited data. Response Timeliness (TA-13/TA-27) is appended per agent, fresh on every response, never persisted — `evaluate_response_timeliness(turns, target_agent_user_id=...)` scopes "worst gap" to that one agent's own replies while still resetting `waiting_since` whenever *any* agent replies (a customer's wait genuinely ends regardless of who answered). IN-12's `top_strength`/`top_gap`/`audit_summary` (`ticket_audit_summary.py`) are computed per agent, from that agent's own findings only. `_payload()` returns `{ticket_id, cached, view_scope, agents: [...]}` — a list, not a singular result — filtered by `ticket_permissions.filter_audits_for_viewer` (a manager sees every agent's card; anyone else sees only their own). Rubric-builder routes (TA-24 follow-on) are unchanged by this rebuild. |
| `ticket_audit_summary.py` | IN-12. `top_strength()`/`top_gap()` (deterministic, no Claude call — highest-weighted pass/fail dimension respectively), `generate_audit_summary()`, and `enrich()` (attaches those three onto a scorecard dict). Each pick carries its own `reasoning`/`evidence_text`/`evidence_seq`/`evidence_verified` (2026-09-20) — the actual quote that made it the standout, not just the dimension name, rendered in `AuditSummaryTiles.tsx` via the same `TicketEvidence` component a per-criterion finding uses (including the "not verified verbatim" flag). Built fresh from CallLoop's own per-dimension scoring output only — never Intercom's `conversation_summary`, never PyAI's Recap. GET ticket / GET mine / POST score / Team Performance call rollups all go through this — Team Performance's Top Strength/Gap is a *mode across many tickets*, though, so it has no single quote to show and stays name-only. |
| `ticket_audit_store.py` | TA-11, rebuilt TA-21/TA-28 (2026-09-16). Org-scoped read/upsert for `ticket_audits`, now one row per `(ticket_id, agent_user_id)` (migration `0039_ticket_audits_per_agent`, which wiped the table's pilot-stage rows outright — a whole-ticket finding doesn't fit the new per-agent schema's meaning, and no fabricated backfill was attempted). `fetch_all()`/`fetch_for_agent()` replace `fetch_latest()`; `upsert_many()` replaces `upsert()` and gives **each agent's row its own `with db.connection() as conn:` block** — a shared connection across the loop would let one agent's `ForeignKeyViolation` rollback discard another agent's already-committed row from earlier in the same loop. `_row_to_dict()` spreads the stored JSONB payload (`**payload`) into the top-level dict rather than nesting it under a `"findings"` key, so a stored row and a freshly-computed `ticket_scoring.score_ticket_per_agent()` result share the same flat shape. Not `audit_store.py` (that is calls). |
| `team_performance.py` | Team Performance Dashboard rollups. Simple org-scoped SQL (no Redis, no job): ticket avgs from `ticket_audits` (one row per agent), call avgs from the latest `audits` row per call joined to `calls.agent_user_id` (IN-22). Ticket and call Top Strength/Gap are the mode of `ticket_audit_summary.top_strength`/`top_gap` across that agent's stored findings in the window (call findings share the same verdict/weight/name shape). Coverage is scored vs total tickets/calls in the window. `heatmap.tickets` / `heatmap.calls` are per-dimension pass rates (`pass / pass+partial+fail`) via `jsonb_array_elements` on id/name/verdict only — Response Timeliness is omitted (not persisted). Heatmap cells include effective KPI `target`/`met` from `performance_kpis` (agent override, else org default). `org.tickets.target` / `org.calls.target` are `__overall__` org defaults. Display names come from `org_members` first/last, never email. Unassigned calls (`agent_user_id` NULL) are a manager-only table bucket with a CTA to JustCall mapping, not a heatmap row. |
| `team_performance_api.py` | `GET /api/team-performance?days=` (1–365, default 30). JWT org_id only. Owner/Manager `view_scope=team`; a member gets `own` and SQL is scoped to their `user_id`. Not `require_owner_or_manager()` — agents can open the page and see themselves. |
| `training.py` | Training drills from stored fail findings for the agent's Top Gap (or a heatmap `dim`+`channel` override). No Claude, no job, no new table. Ticket drills use `reasoning`; call drills prefer `coaching_note`. Cap 5 per channel. Members always load themselves; Owner/Manager may pass an org `agent` UUID. |
| `training_api.py` | `GET /api/training?days=&channel=&dim=&agent=`. JWT org_id only. Invalid `channel`/`dim` → 400. Emits `training_viewed`. |
| `performance_kpis.py` | Assignable Team Performance KPI targets. `performance_kpis` table (migration `0044`): org-default (`agent_user_id` NULL) and per-agent overrides, keyed by live rubric `dimension_id` or `__overall__`. GET merges the active call/ticket rubric so a new custom criterion appears with `target: null` until set. Owner/Manager write; members read org default + own override only. |
| `performance_kpis_api.py` | `GET/PUT /api/performance-kpis`. PUT is `require_owner_or_manager`. `target: null` clears the row. Emits `kpi_target_updated`. |
| `call_agent_identity_aliases.py` | IN-22/23. JustCall `agent_email` → `org_members.user_id` mapping (`call_agent_identity_aliases` table, migration `0042`). `set_alias()` backfills still-unresolved `calls.agent_user_id` in the same transaction. Frontend: `JustCallAgentIdentityMapping` on Integrations (owner-only), parallel to Ticket Audit's Intercom mapper. |
| `call_agent_identity_aliases_api.py` | IN-22/23. `/api/calls/agent-identity-aliases` HTTP surface — GET/POST/DELETE, exact mirror of `ticket_agent_identity_aliases_api.py`'s shape and owner-only tier, calls instead of tickets. Also adds `PATCH /api/calls/{call_id}/agent` (IN-26, owner-or-manager) — the after-the-fact reassignment route, writing an `audit_log` row with the real before/after. |
...
| `password_events.py` | AC-16. Append-only password reset/change trail — the capture point Supabase itself can't provide (its `auth.audit_log_entries` has an `ip_address` column but sits at 0 rows for this project despite real activity, and Auth Hooks have no "password changed" event to attach to). `record_event()` is called from the admin reset actions and from a self-report the frontend sends right after Supabase confirms a self-service change; `history_for_user()` backs the admin console's per-member password-history row. One of the five parallel append-only event-table modules `audit_log.py` (below) now generalizes — see the Backend Blueprint review, §2.4. |
| `audit_log.py` | AC-63/AC-65. The org-facing Activity Log's actual storage — a real security/compliance trail (state-changing actions only: role changed, credential saved, rubric activated, alias mapped, export generated... never plain reads) — separate from `product_events.py` (product analytics) and from `audit_store.py`/`ticket_audit_store.py` (scoring output, a different concept entirely). `record()` is best-effort like `product_events.track_event()` — never raises into the action it's attached to, logs and returns on failure. `actor_id`/`actor_email`/`ip_address` default to whatever's bound on the request-scoped context (`org_ids.py`, AC-64) so most call sites never pass them explicitly. `list_for_org()` (RLS-scoped, AC-69) backs `GET /api/audit-log`; `list_all()` (`bypass_rls`, narrowly scoped, platform-admin only) backs `GET /api/admin/audit-log`. Called from `org_roles.py`, `org_features.py`, `rubric_builder.py`, `ticket_rubric_builder.py`, `ticket_agent_aliases.py`, `ticket_agent_identity_aliases.py`, `call_agent_identity_aliases.py`, `org_vault.py`, `admin_provision.py`, and the export/upload routes in `api.py`. |
| `call_trail.py` | AC-24/AC-26. `record(call_id, org_id, stage, status, *, detail=None, error=None)` — best-effort append to `call_pipeline_events` (never raises; a trail-write failure must not break the pipeline step it describes). Called alongside the existing `applog.event()` calls at each real stage — transcription, per-criterion scoring (via `qa_v8.run_v8_wave`'s injected `on_dimension_event` callback), recap, final audit result, and every `result_served`. `history(call_id, org_id)` reads it back in order for the admin trail viewer. |
| `ticket_trail.py` | Ticket sibling of `call_trail.py`. `record(ticket_id, org_id, stage, status, *, detail=None, error=None)` — best-effort append to `ticket_pipeline_events` (0045). Stages: `parse`, `image_describe`, `agent_resolve`, `scoring`, `criterion:{id}` (per agent, `agent_user_id` in detail, no quotes), `result_served`. `detail["apis"]` is `{method, endpoint}` templates for the HTTP calls that step made (`with_apis()`, no live ids, query strings, or attachment URLs). Wired from `ticket_ingest.ingest_ticket_pdf`, `intercom_ingest._ingest_intercom_object` (dedupe writes nothing), `ticket_scoring.run_ticket_wave_for_agent` (`on_dimension_event`), and `ticket_score_api.score_ticket_route`. `history(ticket_id, org_id)` backs `GET /api/admin/tickets/{ticket_id}/trail`. |
...
| `rate_limit.py` | AC-72. A generous, in-process sliding-window rate limiter — the only app-level throttle in this codebase before this (the sole prior `"rate_limited"` status was for upstream Claude 429s, not our own endpoints). In-process, not Redis-backed, deliberately: `render.yaml` runs this on Render's free plan, a single instance with no horizontal scaling, so there's no state to share across. Resets on every deploy — fine for what it guards (a runaway loop or abuse burst, not a long-term quota). `enforce(bucket, key, *, limit, window_seconds)` raises 429; `key` is typically an org_id, so one org hammering a route can't degrade another org's access to it. Scoped narrow and generous on purpose — a false positive here blocks a paying customer mid-workflow. Current buckets: `call_upload` (30/300s), `call_upload_batch` (10/300s), `ticket_upload` (30/300s), `ticket_score` (60/300s). |
...
| `0024_ticket_audits` | `ticket_audits` (TA-11). Originally one stored scorecard per ticket. RLS SELECT/INSERT/UPDATE. Makes the rescoring guard enforceable. **Superseded by `0039_ticket_audits_per_agent` below (TA-21/TA-28)** — see that row for the current shape; this migration's own DDL is unchanged and still an accurate record of what it did. |
...
| `0039_ticket_audits_per_agent` | **TA-21/TA-28 (2026-09-16).** Widens `ticket_audits` from one row per ticket to one row per `(ticket_id, agent_user_id)` — the schema change behind the per-agent scoring rebuild (see `ticket_scoring.py`/`ticket_audit_store.py` rows above for why). `DELETE FROM ticket_audits` first — pilot-stage data only, and a whole-ticket finding doesn't map onto the new per-agent meaning, so no fabricated backfill was attempted. Adds `agent_user_id UUID NOT NULL REFERENCES org_members (user_id)` and `updated_at TIMESTAMPTZ NOT NULL DEFAULT now()`; drops the old `UNIQUE (ticket_id)` constraint and adds `UNIQUE (ticket_id, agent_user_id)` instead — the actual constraint name (`ticket_audits_ticket_id_key`) was confirmed live via `pg_constraint` before being referenced in DDL, not guessed. No RLS/grant changes — `0024`'s SELECT/INSERT/UPDATE policies already key on `org_id`, which is untouched. |
| `0044_performance_kpis` | Org-default and per-agent KPI targets (`performance_kpis`) for Team Performance, keyed by live rubric `dimension_id` or `__overall__`. RLS SELECT/INSERT/UPDATE/DELETE. Unique on `(org_id, channel, dimension_id, COALESCE(agent_user_id, '00000000-…'))`. |
| `0045_ticket_pipeline_events` | Append-only per-ticket pipeline trail (sibling of `0021_call_pipeline_events`). `ticket_id UUID REFERENCES tickets ON DELETE CASCADE`. GRANT SELECT+INSERT only; RLS org-scoped. Written by `ticket_trail.record()`. |

### `frontend/src/`

| Folder | Purpose |
|---|---|
| `pages/` | One file per route/screen: `Login.tsx` (also handles "Forgot password?", CL-29), `ResetPassword.tsx` (the set-new-password landing page a reset email links to, CL-29 — gated on Supabase's `PASSWORD_RECOVERY` auth event, not just the URL having a token), `Home.tsx`, `FlaggedForReview.tsx`, `ChurnRisk.tsx`, `Training.tsx` (Listen / Drill / Recap from stored Top Gap findings), `TeamPerformance.tsx` (org/self rollup + ticket/call dimension pass-rate heatmap with Met/Missed tags; still fetches `GET /api/performance-kpis` read-only for those tags, but the editing UI lives on `KpiTargets.tsx`), `KpiTargets.tsx` (`/kpi-targets`, linked from `AccountMenu`'s profile dropdown and from Team Performance's header, same role-adaptive-single-page pattern as `RubricBuilder.tsx` — owner/manager get the editable "Set KPIs" panel, `PUT /api/performance-kpis`; a member gets a read-only "Your KPI targets" view of the org default plus their own override, never the roster or teammates' overrides — access is enforced server-side in `performance_kpis.py`/`performance_kpis_api.py`, this split is presentation only), `Integrations.tsx` (its JustCall and Intercom sections each read `enable_justcall_integration`/`enable_intercom_integration` off `AuthContext`'s `features` map and swap in a "turned off for this organization" notice when disabled — presentation only, the real gate is server-side per the route table above), `AgentsPulse.tsx`, `Feedbacks.tsx`, `Neighbourhood.tsx`, `Pyai.tsx`, `Admin.tsx` (platform-admin only; redirects everyone else to `/`, real enforcement is server-side). **AC-37/38/39/40 (2026-09-07): rebuilt as a master-detail directory + slide-over inspector**, replacing the old single-long-scroll layout. `.cc-shell` wrapper (AC-32's Electric Cyan tokens, `App.css`) → a directory table fed by `GET /api/admin/orgs` (AC-33, one row per org) → clicking a row opens a slide-over drawer (`selectedOrg` state, no route change — the table never unmounts, so its scroll position survives the drawer closing) with 4 tabs: **Overview** (org metadata + cost/usage stats + a plain inline-SVG `UsageSparkline` of AC-35's real `/api/admin/usage/daily` series — no chart library), **Flags** (AC-36's `GET /api/admin/feature-flags` metadata groups toggles into Low/Medium/Danger sections; a `danger`-tier toggle opens a confirmation `Modal` before the actual `POST /api/admin/features` call fires, gated generically off `risk` rather than hardcoded key names), **Rubric** (unchanged `GET/POST /api/admin/orgs/{org_id}/rubric` wiring), and **Members** (AC-34's per-member list — `GET /api/admin/directory?q=<org_id>` filtered client-side to that org — each row independently wired to Log in as / Send reset email / an expandable password-history row, all keyed by `user_id` so N members' state never collides). AC-39: the top-bar search is real (`GET /api/admin/orgs?q=`, org-level to match the new table — not the per-member endpoint), ⌘K/Ctrl+K focuses it from anywhere on the page. AC-40: "Provision user" moved from an always-open inline form into a modal triggered by a top-bar button — same fields, same `POST /api/admin/provision-user` body. The Activity date-range lookup section is unchanged and unrelocated — out of scope for this redesign, kept as-is above the new table. `frontend/src/lib/features.ts`'s `TRIAL_FLAGS`/`adminFlagOn`/`TrialFlag` were deleted as dead code once Flags moved to AC-36's backend-served metadata — `flagEnabled`/`FeatureMap` (used by several customer-facing components) are untouched. |
| `components/` | Reusable UI pieces — call playback (`TranscriptPlayer`, `CallWaveform`), layout (`AppLayout`, `Sidebar`), status widgets (`UsageMeter`, `PyaiBadge`, `LiveTicker`), the JustCall keys form (`KeysPanel`), `IntercomWidget` (support Messenger embed, mounted in `AppLayout` for non-admin-host only — a different thing entirely from `pages/Integrations.tsx`'s own Intercom OAuth connect/disconnect card, shipped IN-18; this stale "no Intercom equivalent yet" note is corrected 2026-09-18), `AccountMenu` (the profile dropdown — Profile, Rubric (`/rubric-view`, new tab), KPI targets (`/kpi-targets`, same tab), Sign out). `Sidebar`/`UsageMeter`/`PyaiBadge` all read `AuthContext`'s `features` map to hide themselves per-org; `Sidebar` also renders the caller's own org name under the tagline (CL-31). |
...
| GET | `/api/audit-log` | AC-69. Owner or manager. This org's own Activity Log — real state-changing actions only (`audit_log.list_for_org()`, RLS-scoped), not plain reads. |
| GET | `/api/admin/audit-log` | AC-69. **Platform-admin only.** Every org's Activity Log (`audit_log.list_all()`, `bypass_rls`, narrowly scoped to this one read) — Command Center's cross-org view. |
...
| GET | `/api/admin/calls/{call_id}/trail?org_id=` | **Platform-admin only.** (AC-24/AC-27.) Full pipeline audit trail for one call. `org_id` is a required, caller-supplied query param (Call Logs already has it per row from its own search) — this route must never `bypass_rls` (repo-wide guardrail in `test_rls.py`); every query is scoped through `org_scope(org_id)`. |
| GET | `/api/admin/ticket-logs?query=` | **Platform-admin only.** Search tickets by email, org id, or short id — same resolver as Call logs. Member match is tickets where that user appears as `ticket_messages.agent_user_id`. |
| GET | `/api/admin/tickets/{ticket_id}/trail?org_id=` | **Platform-admin only.** Full pipeline trail for one ticket (`ticket_trail.history`). Same caller-supplied `org_id` / no-`bypass_rls` contract as the call trail. |
...
| GET/POST/DELETE | `/api/integrations/justcall` | JustCall credential status / save / delete. POST is 403 when `enable_justcall_integration` is off for the org; DELETE is never gated, so a disabled org can still remove its own stored credentials. |
| POST | `/api/integrations/justcall/sync` | Manually trigger a JustCall pull. Same `enable_justcall_integration` 403 as the POST above. |
| POST | `/api/integrations/justcall/webhook` | JustCall's inbound webhook (new call recorded) — public, signature-verified |
| GET/DELETE | `/api/integrations/intercom` | Intercom connection status / disconnect. UI: Integrations page Intercom card. DELETE is never gated by `enable_intercom_integration`. |
| GET | `/api/integrations/intercom/connect` | Starts the OAuth flow — 302 to Intercom's authorize URL, `state` HMAC-signed with the caller's org_id. 403 when `enable_intercom_integration` is off for the org (checked after the 503 app-not-configured case). |
| GET | `/api/integrations/intercom/callback` | Public — OAuth redirect target. Exchanges the code, captures the workspace id (`app.id_code`) via `GET /me`, stores the credential, then 302s to the customer Integrations page (IN-18). |
| POST | `/api/integrations/intercom/webhook` | Public, signature-verified (HMAC-SHA1). Resolves org via the payload's `app_id`; dispatches `conversation.admin.closed`/`ticket.resolved`/`ticket.closed`, and (2026-09-18) `app.uninstalled` — deletes that org's stored Intercom credential immediately (via `org_vault.delete_credential`, wrapped in `org_scope()` since this route has no JWT-bound RLS context) so a customer uninstalling from Intercom's own side doesn't leave a dead token for the poller to keep failing against. Webhooks do fire in production (see the 2026-09-12 correction below — this row previously repeated the earlier, stale "never fires" claim) |
...
| GET | `/api/tickets/{ticket_id}` | One ticket, sequenced turns (TA-30: always the full thread, every role), screenshot metadata, and an `audits` list — every agent's scorecard for a manager, only the caller's own otherwise (TA-29). |
| GET | `/api/tickets/{ticket_id}/assets/{seq}` | Signed URL for one stored ticket screenshot |
| GET | `/api/tickets/mine` | TA-12/TA-30. The calling agent's own scorecard rolled up across every ticket they've touched — never a teammate's individual score, even on a shared thread. The full thread comes back for each ticket (not just their own turns); `own_span_seqs` marks which are theirs. |
| GET | `/api/team-performance` | Org-scoped Team Performance rollup (`?days=` 1–365, default 30). Owner/Manager see every teammate (`view_scope=team`); a member sees only their own row (`own`). Ticket and call Top Strength/Gap (mode of `ticket_audit_summary` pickers). Coverage = scored vs total in the window. `heatmap.tickets` / `heatmap.calls` = per-dimension pass rates with effective KPI `target`/`met`. |
| GET | `/api/training` | Org-scoped Training drills (`?days=` 1–365, optional `channel=ticket` or `call`, `dim=`, `agent=`). Members always see themselves; Owner/Manager may pick an org teammate. Latest fail findings for Top Gap (or the heatmap override). |
| GET | `/api/performance-kpis` | Catalog of KPI targets from the live call/ticket rubric plus stored org defaults and per-agent overrides. Members see org defaults and their own overrides only. |
| PUT | `/api/performance-kpis` | Owner or manager. Set or clear one target (`{channel, dimension_id, agent_user_id, target}`). `target: null` unsets. Dimension must be on the current active rubric or `__overall__`. |
| POST | `/api/tickets/{ticket_id}/score` | Score every resolved agent on a ticket independently (TA-10/TA-21/TA-25). Returns `{agents: [...]}` — one scorecard per agent, filtered to the caller's own unless they're a manager. The rescoring guard (TA-11/TA-28) is per agent: a stored row blocks a re-run without `?refresh=true` + `enable_ticket_rescoring` (403 otherwise), but a newly-resolved agent with no row yet always gets a real first score. |
| POST | `/api/tickets/audit-all` | Owner or manager. Scores every ready, not-yet-audited ticket in one call by invoking `score_ticket_route` per ticket — same rate limit, same per-agent skip-if-already-scored logic, no duplicated logic. Capped at 50 tickets per call (stays under `ticket_score`'s 60/300s rate limit) with a bounded 5-worker pool and per-ticket error isolation; returns `{candidates, attempted, scored, errors, remaining}` so the frontend can offer "click again" for the rest. |
...
| GET | `/api/tickets/agent-identity-aliases` | IN-10. Owner-only. Current provider+identifier→user_id mappings (Intercom's `author.email`), unresolved identifiers with a suggested match, and the org roster. Consumed by `TicketAudit.tsx`'s `IntercomIdentityAliasMapping` (stale note removed 2026-09-18 — this has had a frontend since it shipped). |
| POST | `/api/tickets/agent-identity-aliases` | IN-10. Owner-only. Create/update one mapping (`{provider, identifier, user_id}`, `provider` defaults to `"intercom"`). Backfills already-ingested, still-unresolved turns for that exact identifier in the same transaction; future ingestions resolve automatically. |
...
| GET | `/api/calls/agent-identity-aliases` | IN-22/23. Owner-only. Calls' equivalent of the row above — JustCall's `agent_email` instead of Intercom's `author.email`. Same shape (mappings, unresolved identifiers with a suggested match, roster). Consumed by `Integrations.tsx`'s `JustCallAgentIdentityMapping`. |
| POST | `/api/calls/agent-identity-aliases` | IN-22/23. Owner-only. Create/update one mapping (`{provider, identifier, user_id}`, `provider` defaults to `"justcall"`). Backfills already-ingested, still-unresolved calls for that exact identifier in the same transaction. |
| DELETE | `/api/calls/agent-identity-aliases/{provider}/{identifier}` | IN-22/23. Owner-only. Remove a mapping — already-ingested calls keep whatever `agent_user_id` they resolved to. |
| PATCH | `/api/calls/{call_id}/agent` | IN-26. Owner or manager. Reassigns a call's `agent_user_id` after the fact (a manager uploading on someone's behalf, a misrouted JustCall webhook, a call ingested unresolved and identified later) — writes an `audit_log` row with the real before/after. |
...
- **IN-12 Top Strength/Gap/audit_summary render on Ticket Audit and My contributions** — GET `/api/tickets/{id}` and GET `/api/tickets/mine` now attach `ticket_audit_summary.enrich()` (same fields POST `/score` already returned). Team Performance also shows per-agent ticket *and* call Top Strength/Gap (mode across the window). JustCall agent-email mapping UI is on Integrations (`JustCallAgentIdentityMapping`).
- **TA-21/TA-28's per-agent rebuild (2026-09-16) landed backend + tests + `TicketAudit.tsx`/`MyTicketContributions.tsx`, but has not been click-tested in a real browser against a logged-in session** — verified via `tsc -b` (clean) and the full backend/live-DB test suite, but the dev server needs a real Supabase login this session didn't have credentials for. **TA-22 (Launch Gating) was built anyway on explicit instruction (2026-09-16), ahead of its own stated gate** — its own description says "must happen last... only once [the dependency epics] are built AND verified against a real pilot ticket," and that pilot-ticket verification still hasn't happened. `show_ticket_audit_nav` is now on by default for every new org, so this is worth an actual click-through sooner rather than later, not just before some future unblock decision.
- **Intercom OAuth for other companies** is documented in [`docs/intercom-oauth-publish.md`](docs/intercom-oauth-publish.md): the app must be **reviewed** (App Store listing is optional; unlisted is enough); recreating the app is env + Developer Hub settings, not a code rewrite; after approval, new CallLoop orgs use Integrations → Connect Intercom. The app has not been submitted for review yet; OAuth scopes are still broader than ingest needs (a review risk). The OAuth callback **does** redirect to Integrations (IN-18) — it no longer returns JSON.
