...
| `ticket_trail.py` | Ticket sibling of `call_trail.py`. `record(ticket_id, org_id, stage, status, *, detail=None, error=None)` — best-effort append to `ticket_pipeline_events` (0045). Stages: `parse`, `image_describe`, `agent_resolve`, `scoring`, `criterion:{id}` (per agent, `agent_user_id` in detail, no quotes), `result_served`. Wired from `ticket_ingest.ingest_ticket_pdf`, `intercom_ingest._ingest_intercom_object` (dedupe writes nothing), `ticket_scoring.run_ticket_wave_for_agent` (`on_dimension_event`), and `ticket_score_api.score_ticket_route`. `history(ticket_id, org_id)` backs `GET /api/admin/tickets/{ticket_id}/trail`. |
